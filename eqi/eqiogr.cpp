...
eqiOgr::eqiOgr(const QString &fileName)
    : poDataset(nullptr)
{
    // 支持中文路径
...
bool eqiOgr::open(const QString &fileName)
{
    // 注册所有影像格式
    GDALAllRegister();
...
bool eqiOgr::getPixelValue( const QgsPoint& point, double &value )
{
    int iCol = 0;
    int iRow = 0;
...
bool eqiOgr::Projection2ImageRowCol( const QgsPoint& point, int &iCol, int &iRow )
{
    if ((!poDataset) || poDataset->GetRasterCount() == 0)
        return false;
