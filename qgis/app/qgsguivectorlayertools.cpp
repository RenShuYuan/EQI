...
bool QgsGuiVectorLayerTools::startEditing( QgsVectorLayer* layer ) const
{
  ...
  if ( !layer->isEditable() && !layer->isReadOnly() )
  {
    ...
    {
      MainWindow::instance()->messageBar()->pushMessage( tr( "Start editing failed" ),
  ...
  if ( layer->isModified() )
  {
                                       ...
                                       buttons ) )
    {
      ...
      case QMessageBox::Discard:
        MainWindow::instance()->mapCanvas()->freeze( true );
        if ( !layer->rollBack() )
        {
          MainWindow::instance()->messageBar()->pushMessage( tr( "Error" ),
