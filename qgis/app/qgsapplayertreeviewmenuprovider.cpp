...
QMenu* QgsAppLayerTreeViewMenuProvider::createContextMenu()
{
  ...
  if ( !idx.isValid() )
  {
    ...
    menu->addAction( eqiApplication::getThemeIcon( "mActionExpandTree.svg" ), tr( "&Expand All" ), mView, SLOT( expandAll() ) );
    menu->addAction( eqiApplication::getThemeIcon( "mActionCollapseTree.svg" ), tr( "&Collapse All" ), mView, SLOT( collapseAll() ) );
  ...
  else if ( QgsLayerTreeNode* node = mView->layerTreeModel()->index2node( idx ) )
  {
    ...
    if ( QgsLayerTree::isGroup( node ) )
    {
	  ...
	  // 移除
      menu->addAction( eqiApplication::getThemeIcon( "mActionRemoveLayer.svg" ), tr( "&Remove" ), MainWindow::instance(), SLOT( removeLayer() ) );

	  // 设置组的坐标参照系
      menu->addAction( eqiApplication::getThemeIcon( "mActionSetCRS.png" ),
                       tr( "&Set Group CRS" ), MainWindow::instance(), SLOT( legendGroupSetCRS() ) );
    ...
    else if ( QgsLayerTree::isLayer( node ) )
    {
      ...
      {
		// 缩放到实际像素
        menu->addAction( eqiApplication::getThemeIcon( "mActionZoomActual.svg" ), tr( "&Zoom to Native Resolution (100%)" ), MainWindow::instance(), SLOT( legendLayerZoomNative() ) );

		// 按当前画布范围拉伸
        if ( rlayer->rasterType() != QgsRasterLayer::Palette )
          menu->addAction( tr( "&Stretch Using Current Extent" ), MainWindow::instance(), SLOT( legendLayerStretchUsingCurrentExtent() ) );
      }
      ...
      // 创建副本
      QAction* duplicateLayersAction = menu->addAction( eqiApplication::getThemeIcon( "mActionDuplicateLayer.svg" ), tr( "&Duplicate" ), MainWindow::instance(), SLOT( duplicateLayers() ) );

	  //判断是否是有效几何图形(点、线、面)
      if ( !vlayer || vlayer->geometryType() != QGis::NoGeometry )
      {
        // 设置图层可视性比例
        menu->addAction( tr( "&Set Layer Scale Visibility" ), MainWindow::instance(), SLOT( setLayerScaleVisibility() ) );

        // 设置图层参考坐标系
        menu->addAction( eqiApplication::getThemeIcon( "mActionSetCRS.png" ), tr( "&Set Layer CRS" ), MainWindow::instance(), SLOT( setLayerCRS() ) );

        // 应用图层的参考坐标系到项目
        menu->addAction( eqiApplication::getThemeIcon( "mActionSetProjectCRS.png" ), tr( "Set &Project CRS from Layer" ), MainWindow::instance(), SLOT( setProjectCRSFromLayer() ) );
      }
      ...
      {
        if ( MainWindow::instance()->clipboard()->hasFormat( QGSCLIPBOARD_STYLE_MIME ) )
        {
          menu->addAction( tr( "Paste Style" ), MainWindow::instance(), SLOT( applyStyleToGroup() ) );
        }
      }
      ...
      if ( vlayer )
      {
        ...
        // attribute table
        menu->addAction( eqiApplication::getThemeIcon( "mActionOpenTable.svg" ), tr( "&Open Attribute Table" ),
                         MainWindow::instance(), SLOT( attributeTable() ) );
      ...
      {
        menu->addAction( tr( "Save As..." ), MainWindow::instance(), SLOT( saveAsRasterFile() ) );
        menu->addAction( tr( "Save As Layer Definition File..." ), MainWindow::instance(), SLOT( saveAsLayerDefinition() ) );
      }
      ...
      if ( layer && QgsProject::instance()->layerIsEmbedded( layer->id() ).isEmpty() )
        menu->addAction( tr( "&Properties" ), MainWindow::instance(), SLOT( layerProperties() ) );
  ...
  else if ( QgsLayerTreeModelLegendNode* node = mView->layerTreeModel()->index2legendNode( idx ) )
  {
    if ( QgsSymbolV2LegendNode* symbolNode = dynamic_cast< QgsSymbolV2LegendNode* >( node ) )
    {
      ...
      {
        menu->addAction( eqiApplication::getThemeIcon( "mActionShowAllLayers.png" ), tr( "&Show All Items" ),
                         symbolNode, SLOT( checkAllItems() ) );
        menu->addAction( eqiApplication::getThemeIcon( "mActionHideAllLayers.png" ), tr( "&Hide All Items" ),
