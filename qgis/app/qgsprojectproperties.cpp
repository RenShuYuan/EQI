...

{
  ...
  // 重新定义图标及大小
  pbnAddScale->setIcon(eqiApplication::getThemeIcon("symbologyAdd.svg"));
  pbnRemoveScale->setIcon(eqiApplication::getThemeIcon("symbologyRemove.svg"));
  pbnImportScales->setIcon(eqiApplication::getThemeIcon("mActionFileOpen.svg"));
  pbnExportScales->setIcon(eqiApplication::getThemeIcon("mActionFileSave.svg"));
  for (int i=0; i<mOptionsListWidget->count(); ++i)
  {
	  QListWidgetItem *item = mOptionsListWidget->item(i);
	  if (0==i)
	  {
          item->setIcon(eqiApplication::getThemeIcon("propertyicons/general.svg"));
	  } 
	  else if (1==i)
	  {
          item->setIcon(eqiApplication::getThemeIcon("propertyicons/CRS.svg"));
	  }
	  else if (2==i)
	  {
          item->setIcon(eqiApplication::getThemeIcon("propertyicons/map_tools.png"));
	  }
  }
