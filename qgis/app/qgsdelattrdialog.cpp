    ...
    for ( int idx = 0; idx < layerAttributes.count(); ++idx )
    {
      ...
      switch ( vl->fields().fieldOrigin( idx ) )
      {
        case QgsFields::OriginExpression:
          item->setIcon( eqiApplication::getThemeIcon( "mIconExpression.svg" ) );
          break;

        case QgsFields::OriginJoin:
          item->setIcon( eqiApplication::getThemeIcon( "propertyicons/join.png" ) );
          item->setFlags( item->flags() & ~Qt::ItemIsEnabled );
          break;

        default:
          item->setIcon( eqiApplication::getThemeIcon( "propertyicons/attributes.png" ) );
