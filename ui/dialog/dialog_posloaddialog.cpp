...
dialog_posloaddialog::dialog_posloaddialog(QWidget *parent)
    : QDialog(parent),
      mExampleRowCount(10),
      mBadRowCount(0),
...
dialog_posloaddialog::~dialog_posloaddialog()
{
    delete mFile;
}


void dialog_posloaddialog::on_btnOpenPos_clicked()
{
    getOpenFileName();
}

void dialog_posloaddialog::on_buttonBox_accepted()
{
    if ( ! mFile->isValid() )
    {
...
void dialog_posloaddialog::getOpenFileName()
{
    // 获取文件的过程，并从当前目录设置初始目录到最后使用
    QString posFileName = QFileDialog::getOpenFileName(this,
        "打开POS文件",
        mSettings.value("/eqi/pos/pathName", QDir::homePath()).toString(),
        "POS (*.txt *.pos *.gps)");
...
void dialog_posloaddialog::on_lePosFile_textChanged()
{
    ...
    {
        QSettings settings;
        mSettings.setValue("/eqi/pos/pathName", QDir::fromNativeSeparators(file_info.path()));
    }

    updateFieldLists();
}

void dialog_posloaddialog::updateFieldLists()
{
        ...
        for ( int i = 0; i < ui.tableWidget->columnCount(); i++ )
        {
            ...
            if ( ! value.isEmpty() )
            {
                ...
                if ( isValidCoordinate[i] )
                {
                    ...
                    if ( xyDms )
                    {
                        QRegExp CrdDmsRegexp("^\\s*(?:([-+nsew])\\s*)?(\\d{1,3})"
                                             "(?:[^0-9.]+([0-5]?\\d))?[^0-9.]+([0-5]"
                                             "?\\d(?:\\.\\d+)?)[^0-9.]*([-+nsew])?\\s*$",
                                             Qt::CaseInsensitive);
...
QString dialog_posloaddialog::selectedChars()
{
    QString chars = "";
...
void dialog_posloaddialog::loadSettings()
{

    bool blchk = mSettings.value("/eqi/pos/chkComma", false).toBool();
    ui.chkComma->setChecked(blchk);
    blchk = mSettings.value("/eqi/pos/chkTab", true).toBool();
    ui.chkTab->setChecked(blchk);
    blchk = mSettings.value("/eqi/pos/chkSpace", false).toBool();
    ui.chkSpace->setChecked(blchk);
    blchk = mSettings.value("/eqi/pos/chkColon", false).toBool();
    ui.chkColon->setChecked(blchk);
    blchk = mSettings.value("/eqi/pos/chkSemicolon", false).toBool();
    ui.chkSemicolon->setChecked(blchk);

    blchk = mSettings.value("/eqi/pos/cbxUseHeader", true).toBool();
    ui.cbxUseHeader->setChecked(blchk);
    int num = mSettings.value("/eqi/pos/rowCounter", 0).toInt();
    ui.rowCounter->setValue(num);

    blchk = mSettings.value("/eqi/pos/chkClipSpace", true).toBool();
    ui.chkClipSpace->setChecked(blchk);
    blchk = mSettings.value("/eqi/pos/cbxSkipEmptyFields", false).toBool();
    ui.cbxSkipEmptyFields->setChecked(blchk);
    blchk = mSettings.value("/eqi/pos/chkDMSformat", false).toBool();
    ui.chkDMSformat->setChecked(blchk);

    QString str = mSettings.value("/eqi/pos/lePosFile", "").toString();
    ui.lePosFile->setText(QDir::toNativeSeparators(str));

}

void dialog_posloaddialog::saveSettings()
{
    mSettings.setValue("/eqi/pos/chkComma", ui.chkComma->isChecked());
    mSettings.setValue("/eqi/pos/chkTab", ui.chkTab->isChecked());
    mSettings.setValue("/eqi/pos/chkSpace", ui.chkSpace->isChecked());
    mSettings.setValue("/eqi/pos/chkColon", ui.chkColon->isChecked());
    mSettings.setValue("/eqi/pos/chkSemicolon", ui.chkSemicolon->isChecked());

    mSettings.setValue("/eqi/pos/cbxUseHeader", ui.cbxUseHeader->isChecked());
    mSettings.setValue("/eqi/pos/rowCounter", ui.rowCounter->value());

    mSettings.setValue("/eqi/pos/chkClipSpace", ui.chkClipSpace->isChecked());
    mSettings.setValue("/eqi/pos/cbxSkipEmptyFields", ui.cbxSkipEmptyFields->isChecked());
    mSettings.setValue("/eqi/pos/chkDMSformat", ui.chkDMSformat->isChecked());

    mSettings.setValue("/eqi/pos/lePosFile", QDir::fromNativeSeparators(ui.lePosFile->text()));

    mSettings.setValue("/eqi/pos/fieldsList/cmb1", ui.cmb_1->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb2", ui.cmb_2->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb3", ui.cmb_3->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb4", ui.cmb_4->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb5", ui.cmb_5->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb6", ui.cmb_6->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb7", ui.cmb_7->currentIndex());
    mSettings.setValue("/eqi/pos/fieldsList/cmb8", ui.cmb_8->currentIndex());
}
